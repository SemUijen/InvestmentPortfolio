...
def main():
    ...
    df_new_daily_results = (
        df_new_io_bought.select(
            F.col("symbol"),
            (F.col("price") * F.col("number_owned") * F.col("exchange_rate")).alias(
                "total_investment"
            ),
            (F.col("exchange_rate") * F.col("number_owned")).alias(
    ...
    df_new_daily_results = (
        ...
        .select(
            ...
            (
                F.col("total_value")
                - F.col("local_total_investment") * F.col("currency_exchange_rate")
            ).alias(
                "product_profit",
            ),
            (
                F.col("local_total_investment") * F.col("currency_exchange_rate")
                - F.col("total_investment")
            ).alias(
                "currency_profit",
            ),
            (F.col("total_value") - F.col("total_investment")).alias(
                "unrealized_total_profit"
            ),
        )
        .drop(
            "currency_exchange_rate_change",
            "avg_exchange_rate_change",
            "exchange_rate_weighted",
            "cost_of_buy",
        )
